            ...
            <table class="min-w-full border border-blue-100 rounded-lg overflow-hidden">
                <thead class="bg-blue-50 text-blue-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold border-b">Tanggal</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold border-b">Penulis</th>
                ...
                <tbody class="text-gray-700">
                    @forelse($beritas as $index => $berita)
                        <tr class="hover:bg-blue-50">
                            ...
                            <td class="px-6 py-3 border-b font-medium">{{ $berita->judul }}</td>
                            <td class="px-6 py-3 border-b">{{ $berita->created_at->format('d M Y') }}</td>
                            <td class="px-6 py-3 border-b">{{ $berita->penulis ?? '-' }}</td>
                               <td class="px-6 py-3 border-b">
                                @if($berita->gambar)
                                    <img src="{{ asset('storage/' . $berita->gambar) }}" class="w-20 h-12 object-contain">
                                @else
                                    -
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-gray-500">Belum ada data berita.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
